...
def extract_rfc_and_name_from_text(full_text: str, detected_bank=None):
    ...
    # Banbajio fallback: Nombre is between "R.F.C." line and the bank legal-name line
    # (e.g. "PKF MUÑOZ LAMBARRI SC" before "BANCO DEL BAJIO S.A., INSTITUCION DE BANCA MULTIPLE.").
    if name is None and detected_bank == 'Banbajío':
        rfc_label_re = re.compile(r'\bR\.?\s*F\.?\s*C\.?\b', re.IGNORECASE)
        banbajio_bank_line_re = re.compile(
            r'BANCO\s+DEL\s+BAJ[ÍI]O|INSTITUCI[ÓO]N\s+DE\s+BANCA\s+M[ÚU]LTIPLE',
            re.IGNORECASE
        )
        for i, line in enumerate(lines):
            line_stripped = (line or '').strip()
            if not line_stripped:
                continue
            if not rfc_label_re.search(line_stripped):
                continue
            _name_debug("NOMBRE_CHECK (Banbajio RFC marker): %s" % (line_stripped[:200] + '...' if len(line_stripped) > 200 else line_stripped))
            collected_parts = []
            for j in range(i + 1, len(lines)):
                candidate = (lines[j] or '').strip()
                if not candidate:
                    continue
                m_bank = banbajio_bank_line_re.search(candidate)
                if m_bank:
                    before_bank = candidate[:m_bank.start()].strip(' :,-')
                    if before_bank:
                        collected_parts.append(before_bank)
                    break
                # Ignore repeated RFC label lines that may appear due to OCR noise.
                if rfc_label_re.search(candidate):
                    continue
                collected_parts.append(candidate)
            if collected_parts:
                name = re.sub(r'\s+', ' ', ' '.join(collected_parts)).strip(' :,-')
                if name:
    ...
    if detected_bank == 'Banorte':
        marker = 'estado de cuenta / enlace'
        for i, line in enumerate(lines):
            if marker not in line.strip().lower():
                continue
            for j in range(i + 1, len(lines)):
...
def extract_movement_row(words, columns, bank_name=None, date_pattern=None, debug_only_if_contains_iva=False):
    ...
    if bank_name == 'BBVA' and len(sorted_words) > 1 and columns:
        ...
        i = 0
        while i < len(sorted_words):
            current = sorted_words[i].copy()
            cur_text = (current.get('text') or '').strip()
            merged = False
            if i + 1 < len(sorted_words) and re.match(r'^\d{1,3}$', cur_text):
                nxt = sorted_words[i + 1]
                nxt_text = (nxt.get('text') or '').strip()
                # Next token already has decimal part (e.g. 290.00 / 172.42)
                if re.match(r'^\d{3}(?:[\.,]\d{2})$', nxt_text):
                    cur_center = (current.get('x0', 0) + current.get('x1', 0)) / 2
                    ...
                    if abs(nxt_center - cur_center) <= 220:
                        ...
                        if (
                            ...
                            current['x0'] = nxt.get('x0', current.get('x0', 0))
                            current['x1'] = nxt.get('x1', current.get('x1', 0))
                            combined_words.append(current)
                            i += 2
                            merged = True
            if not merged:
                combined_words.append(current)
...
def main():
    ...
    def _banamex_new_fmt_log(orig, row_data, disp):
        """No-op for Banamex new format (debug prints removed)."""
        return
                ...
                if movement_end_pattern:
                    ...
                    if match_found:
                        ...
                        if bank_config['name'] == 'BBVA' and in_bbva_movements_section:
                            in_bbva_movements_section = False
                        # Banamex new format: validation totals from "Total +" / "Total" lines only; treat movements_end like other banks
